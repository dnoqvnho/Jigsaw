...
#proj = (0.01, 0.05, 0.1, 0.5)
project_a_num = (1, 4, 16, 80)
select_a_num = 1
qt_num = 2

qt = []
for p in project_a_num:
    qt.append([QTemplate(ANUM, select_a_num, p, ranges) for _ in range(qt_num)])
...
for i in range(len(project_a_num)):
    proj = project_a_num[i]
    ofile = open("{}/{}/query_train".format(sys.argv[3], proj), 'w')
    for _ in range(0, QNUM_train):
        t = qt[i][random.randint(0, qt_num - 1)]
        q = t.make_one_query(selectivity)
        ofile.write(q.to_string() + "\n")
    ofile.close()
    
    ofile = open("{}/{}/query_test".format(sys.argv[3], proj), 'w')
    for _ in range(0, QNUM_test):
        t = qt[i][random.randint(0, qt_num - 1)]
        q = t.make_one_query(selectivity)
        ofile.write(q.to_string() + "\n")
    ofile.close()
